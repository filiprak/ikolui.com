...
```vue title="[[_Source code_]]" preview
<template>
  <div class="ik-flex ik-flex-col ik-gap-4" style="max-width: 300px;">
    <IkResource copyable>
...
When no actions are provided (no `copyable`, `uri`, or `action` slot), the component displays as a simple container without hover effects.

```vue title="[[_Source code_]]" preview
<template>
  <IkResource>
    Simple text without actions
  </IkResource>
</template>
<script setup>
import { IkResource } from '@ikol/ui-kit/components/IkResource';
...
A complete example combining multiple features for a resource management interface.

```vue title="[[_Source code_]]" preview
<template>
  <div class="ik-flex ik-flex-col ik-gap-6">
    <div>
      <h4 class="ik-text--sm ik-mb-2">Website</h4>
      <IkResource 
        uri="https://example.com" 
        copyable
        show_actions
    ...
    <div>
      <h4 class="ik-text--sm ik-mb-2">Email</h4>
      <IkResource 
        uri="contact@example.com" 
        email
        copyable
        show_actions
      >
    ...
    <div>
      <h4 class="ik-text--sm ik-mb-2">Phone</h4>
      <IkResource 
        uri="+1234567890" 
        phone
        copyable
