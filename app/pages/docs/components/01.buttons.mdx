...
<template>
  ...
  <IkButton size="xl" design="primary">Extra large</IkButton>
</template>
<script setup>
import { IkButton } from '@ikol/ui-kit/components/IkButton';
</script>
...
Add an icon before the button label using the `prepend_icon` prop.
...
```vue title="[[_Source code_]]" preview
<template>
  <IkButton design="primary" fab icon="plus" class="ik-mr-4"></IkButton>
  <IkButton design="success" fab icon="check" class="ik-mr-4"></IkButton>
  <IkButton design="accent" fab icon="edit"></IkButton>
</template>
<script setup>
import { IkButton } from '@ikol/ui-kit/components/IkButton';
</script>
