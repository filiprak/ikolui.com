...
The component emits several events for tracking user interactions:

- `update:modelValue`: Fired when the quantity value changes
- `ik-change-quantity`: Fired when the input loses focus (blur event)
- `ik-increment-quantity`: Fired when the increment button is clicked
- `ik-decrement-quantity`: Fired when the decrement button is clicked
...
<template>
  <div>
    <IkQuantityInput
      ...
      @ik-change-quantity="onChange"
      @ik-increment-quantity="onIncrement"
      @ik-decrement-quantity="onDecrement"
      class="ik-mb-4"
    />
    <div class="ik-mt-4">
...
const onChange = (value) => {
  lastEvent.value = `ik-change-quantity - ${value}`;
};

const onIncrement = () => {
  lastEvent.value = 'ik-increment-quantity';
};

const onDecrement = () => {
  lastEvent.value = 'ik-decrement-quantity';
};
</script>
```
...
When the input field loses focus and the value is empty or invalid, it automatically corrects to the minimum value.

```vue title="[[_Source code_]]" preview
<template>
  <div>
    <IkQuantityInput
      v-model="autoCorrectQty"
      :min="1"
      :max="100"
      class="ik-mb-4"
    />
    <p class="ik-text--sm ik-text--default-light">
      Try clearing the input and clicking away - it will auto-correct to 1 (min value)
    </p>
  </div>
</template>
<script setup>
